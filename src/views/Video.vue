<template>
  <div class="video">
    <div v-if="!isLoading">
      ...
      <video-list :videoList="videos"
                  v-if="videos.length !== 0"></video-list>
      <div class="button-wrapper">
        <div class="prev button"
             @click="prevPage"
             :class="{'active-button' : (offset === 0)}">上一页</div>
        <div class="next button"
             @click="nextPage"
             :class="{'active-button': !more}">下一页</div>
      </div>
    </div>
...
export default {
  name: 'Video',
  components: { TagBar, VideoList, Loading },
  data () {
    return {
      // 视频列表
      videos: [],
